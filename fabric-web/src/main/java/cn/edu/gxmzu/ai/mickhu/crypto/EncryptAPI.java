...
public class EncryptAPI {
    ...
    public EncryptAPI(BigInteger p, BigInteger q) {
        RSA = new RSA(p, q);
    }
     ...
     */
    public ShareMessage encryptKeyByRsa(ShareMessage shareMessage) {
        // 对消息体内的AES对应的大数进行加密
        BigInteger key = RSA.encrypt(shareMessage.getBigIntKey());
     ...
     * RSA解密AES秘钥
     */
    public ShareMessage decryptKey(ShareMessage shareMessage) {
        BigInteger key = RSA.decrypt(shareMessage.getBigIntKey());
        // 重新设置AES秘钥
        shareMessage.setKey(key);
        return shareMessage;
    }
